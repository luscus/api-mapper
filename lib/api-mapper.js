...
exports.map = function map (apiAbsolutePath, callback) {
  ...
  crawler.crawlTree(apiAbsolutePath, {});

  crawler.on('data', function processApiParts (data) {
    data = JSON.parse(data);

    if (!data.is.directory) {
      var filepath  = data.path.replace(apiAbsolutePath, '');
...
exports.getApiInfo = function getApiInfo(apiAbsolutePath) {
  ...
  var packageInfo = require(apiAbsolutePath + path.sep + '..' + path.sep + 'package.json');
